...
**文件**: `internal/app/migrator.go`

Migrator 是 AP 模式独有的后台数据迁移组件，负责在 AP 节点扩缩容导致一致性哈希环变化时，将**不再归属于本节点的 key** 迁移到正确的目标节点。
...
│                     每 5 秒触发一次                           │
│                                                             │
│  1. 从 Etcd 拉取当前所有 AP 节点列表                         │
│           │                                                 │
│  2. 构建一致性哈希环（150 虚拟节点/物理节点）                  │
│           │                                                 │
│  3. Scan 本地引擎，对每个 key 执行：                         │
│     target = ring.Get(key)                                  │
│     if target != selfAddr {                                 │
│         加入待迁移队列                                       │
│     }                                                       │
...
│           │                                                 │
│  5. 对每个目标节点批量 Push：                                │
│     ├─ gRPC InternalSet(key, value) 写入目标节点             │
│     ├─ 推送成功后本地 Delete(key)（cut-over）               │
│     └─ 每条 key 延迟 1ms（避免压垮目标节点）                  │
└─────────────────────────────────────────────────────────────┘
```
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `tickInterval` | 5s | 迁移检查周期 |
| `batchSize` | 100 | 每批次最大推送数量 |
| `delayPerItem` | 1ms | 单条 key 推送后的节流延迟 |
| `deleteAfterPush` | true | 推送成功后是否本地删除（cut-over） |

**工作时机：**

- **触发场景**：AP 节点**扩容**时（新节点加入集群 → 一致性哈希环变化 → 部分 key 的归属节点发生改变）
- **启动时机**：Server 启动流程第 9 步，仅 AP 模式节点启动 Migrator（CP 模式由 Raft 保证一致性，无需迁移）
- **持续运行**：后台 goroutine 每 5 秒检查一次，直到节点停止

**注意事项：**

- **缩容不自动处理**：节点下线后无法从其拉取数据，缩容需配合副本机制或提前手动迁移
- **幂等性**：`InternalSet` 是幂等操作，同一 key 多次迁移不会产生副作用
- **与 AOF 的关系**：迁移过程不写入 AOF（目标节点收到 `InternalSet` 后会自行触发 AOF）
- **并发安全**：`engine.Scan` 在分片锁保护下执行，`Delete` 逐条操作，不会阻塞正常读写
