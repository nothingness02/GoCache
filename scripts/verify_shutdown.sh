...
if grep -q "Etcd registry closed" "$SERVER_LOG"; then
  log_success "✓ Server Etcd registry closed"
  PASS_COUNT=$((PASS_COUNT + 1))
else
  log_fail "✗ Server did not close Etcd registry"
  FAIL_COUNT=$((FAIL_COUNT + 1))
fi

if grep -q "Server stopped" "$SERVER_LOG"; then
  log_success "✓ Server stopped gracefully"
  PASS_COUNT=$((PASS_COUNT + 1))
else
  log_fail "✗ Server did not stop gracefully"
  FAIL_COUNT=$((FAIL_COUNT + 1))
fi
